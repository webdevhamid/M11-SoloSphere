...
const MyBids = () => {
  ...
  useEffect(() => {
    const getBids = async () => {
      try {
        const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/bid/${user?.email}`);
        setBids(data);
      } catch (err) {
        console.log(err);
  ...
  //   const getJobs = async () => {
  //     try {
  //       const { data } = await axios.get(`${import.meta.env.VITE_API_URL}/job/${user?.email}`);
  //       setBids(data);
  //     } catch (err) {
  //       console.log(err);
  //       toast.error(err.message);
  ...
  return (
    <section className="container px-4 mx-auto my-12">
      <div className="flex items-center gap-x-3">
        ...
        <span className="px-3 py-1 text-xs text-blue-600 bg-blue-100 rounded-full ">6 Bid</span>
      </div>
